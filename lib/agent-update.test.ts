...
describe("Entitlement is not an Agent field", () => {
    it("ignores plan, override and quota named on an Agent update", () => {
        // Entitlement lives on the account and is resolved server-side. Even
        // if one of these ever became a column, it is not on the allowlist.
        const update = pickAgentUpdate({
            ...agentFromTheApi,
            planTier: "enterprise",
            agentSlotOverride: 100,
            quota: 100,
            effectiveLimit: 100,
            agentCredits: 100,
        }) as Record<string, unknown>;

        expect(update).not.toHaveProperty("planTier");
        expect(update).not.toHaveProperty("agentSlotOverride");
